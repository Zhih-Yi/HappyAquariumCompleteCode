...
<div class="pt-5">
  ...
  <div class="table-responsive">
    <table class="table table-striped table-sm mt-4 text-third" >
      ...
      <tbody>
        <tr v-for="item in todayOrders" :key="item.id">
          ...
          <td>
            <button type="button" class="btn btn-outline-third btn-sm" @click="OpenModal(item)">檢視</button>
          </td>
        </tr>
      </tbody>
    </table>
    <div v-if="todayOrders.length==0&&!isLoading">
      <h3 class="text-center">今日尚無訂單</h3>
    </div>
    <!--今日訂單Modal---->
    <div class="modal fade"  id="TodayOrderModal" tabindex="-1"  aria-hidden="true">
...
export default {
          ...
          for (let i = 2; i < totalPage + 1; i++) {
            ...
            vm.$http.get(apiurl).then((res) => {
              ...
              if (counter === totalPage) {
                ...
                vm.todayOrders = items.filter(item => {
  ...
  mounted () {
    this.ViewModal = new Modal(document.getElementById('TodayOrderModal'))
  }
}
</script>
