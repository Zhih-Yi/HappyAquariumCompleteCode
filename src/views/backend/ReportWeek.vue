...
<div class="pt-5">
  ...
  <h2 class="mt-4 manage-header">本周訂單一覽<i class="fas fa-calendar-minus ms-2"></i></h2>
  <div class="table-responsive">
    <table class="table table-striped table-sm mt-4 text-third">
      ...
      <tbody >
        <tr v-for="item in WeekOrders" :key="item.id">
          ...
          for (let i = 2; i < totalPage + 1; i++) {
            ...
            vm.$http.get(apiurl).then((res) => {
              ...
              if (counter === totalPage) {
                ...
                vm.WeekOrders = items.filter((item) => moment(item.create_at * 1000).isBetween(fromDate, toDate))
