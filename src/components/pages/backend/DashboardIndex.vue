...
<div class="pt-5 pb-3">
  ...
  <div class="row py-4">
    <DisplayBox :count="filterToday.length" :title="'本日訂單'" :bg="'bg-use'" @showModal="openTodayModel"/>
...
<script>
import DisplayBox from '@/components/backend/DisplayBox'
...
export default {
  ...
  methods: {
    ...
    getWeekOrder () {
      const fromDate = moment().startOf('week')
      const toDate = moment().endOf('week')
      this.chartOrders = this.ordersAll.filter((item) => {
        return moment(item.create_at * 1000).isBetween(fromDate, toDate)
      })
      this.setWeekChart()
    },
