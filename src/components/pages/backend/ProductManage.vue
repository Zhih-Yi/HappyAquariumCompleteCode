...
<div class="pt-5">
  ...
  <h2 class="mt-4 manage-header">商品管理<i class="fas fa-shopping-cart ms-2"></i></h2>
  <div class="text-end py-3">
    <button class="btn btn-info text-white" data-toggle="modal" @click="openModal(true)">
    ...
    <table class="table table-striped table-sm mt-4 text-third">
      ...
      <tbody>
        <tr v-for="item in products" :key="item.id" v-cloak>
          ...
          <td><span v-if="item.is_enabled">啟用</span><span v-else>未啟用</span></td>
          <td><button class="btn btn-outline-third btn-sm" @click="openModal(false,item)">編輯</button>
              <button class="btn btn-outline-alert btn-sm" @click.prevent="openModalDelete(item)">刪除</button>
          </td>
        </tr>
      </tbody>
    </table>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header bg-danger bg-gradient">
          <h5 class="modal-title text-white" >刪除商品<i class="fas fa-trash-alt ms-2"></i></h5>
      ...
      <div class="modal-dialog modal-dialog-scrollable modal-xl">
        <div class="modal-content">
          <div class="modal-header bg-abstract text-white bg-gradient">
            <h5 class="modal-title" >{{ modaltitle }}<i class="ms-2 fas fa-pencil-alt" ></i></h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <form class="row g-3">
              <div class="col-12">
                <ValidationProvider rules="required" v-slot="{ errors, classes }">
                  ...
                  <input type="text" name="標題" id="title" v-model="currentProduct.title" placeholder="請輸入標題" class="form-control" :class="classes">
              ...
              <div class="col-md-6">
                <ValidationProvider rules="required" v-slot="{ errors, classes }">
                  ...
                  <select name="分類" id="category" v-model="currentProduct.category" class="form-select" :class="classes">
            ...
            <div class="col-md-6">
              <ValidationProvider rules="required" v-slot="{ errors, classes }">
                ...
                <input type="text" name="單位" id="unit" v-model="currentProduct.unit" placeholder="請輸入單位" class="form-control" :class="classes">
                <span class="invalid-feedback" >{{ errors[0] }}</span>
              </ValidationProvider>
            </div>
            <div class="col-md-6">
              <ValidationProvider rules="required" v-slot="{ errors, classes }">
                ...
                <input type="number" name="原價" id="origin_price" v-model="currentProduct.origin_price" placeholder="請輸入原價" class="form-control" :class="classes">
                <span class="invalid-feedback" >{{ errors[0] }}</span>
              </ValidationProvider>
            </div>
            <div class="col-md-6">
              <ValidationProvider rules="required" v-slot="{ errors, classes }">
                ...
                <input type="number" name="售價" id="price" v-model="currentProduct.price" placeholder="請輸入售價" class="form-control" :class="classes">
                <span class="invalid-feedback" >{{ errors[0] }}</span>
              </ValidationProvider>
            </div>
            <div class="col-12">
              <ValidationProvider rules="required" v-slot="{ errors, classes }">
                ...
                <input type="text" name="圖片" id="image" v-model="currentProduct.imageUrl" placeholder="請輸入圖片連結或上傳圖片" class="form-control" :class="classes">
                <span class="invalid-feedback" >{{ errors[0] }}</span>
              </ValidationProvider>
            </div>
            <div class="col-md-6">
            ...
            <div class="col-md-6">
              <ValidationProvider rules="required" v-slot="{ errors, classes }">
                ...
                <textarea  name="產品描述" id="description" rows="9" v-model="currentProduct.description" placeholder="請輸入產品描述" class="form-control" :class="classes"></textarea>
                <span class="invalid-feedback" >{{ errors[0] }}</span>
              </ValidationProvider>
            </div>
            <div class="col-12">
              <ValidationProvider rules="required" v-slot="{ errors, classes }">
                ...
                <textarea  name="說明內容" id="content" rows="10" v-model="currentProduct.content" placeholder="請輸入產品說明內容" class="form-control" :class="classes"></textarea>
                <span class="invalid-feedback" >{{ errors[0] }}</span>
              </ValidationProvider>
            </div>
...
<script>
import Pagination from '@/components/backend/PaginationManage'
