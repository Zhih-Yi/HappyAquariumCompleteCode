<template>
  <div id="shoppingCart" class="d-flex align-items-center order-md-1">
    <a class="cart nav-link" @click.prevent="cartVisible=!cartVisible">
      ...
      <span class="cart-num badge rounded-pill bg-danger text-white">
        <small>{{ tmpCart.length }}</small>
      </span>
    </a>
      ...
      <div class="cart-content bg-white rounded py-2 px-2" v-if="cartVisible && checkoutVisible">
        <form>
            ...
            <table class="table table-sm" v-if="tmpCart.length>0">
              ...
              <tbody>
                <tr v-for="item in tmpCart" :key="item.id">
                  <td valign="middle">
                    <button type="button" @click.stop="deleteTmpCartItem(item.id)" class="btn p-0 text-third d-flex align-items-center" v-if="checkoutVisible">
                  ...
                  <div class="input-group input-group-sm input-group-fixed">
                    <button class="btn border" @click="minusQuantity(item)" type="button" :id="`minus-${item.id}`">
                      <strong class="h5">−</strong>
                    </button>
                    <input type="number" class="form-control" v-model.number="item.qty" @change="verifyNumber(item)">
                    <button class="btn border" @click="addQuantity(item)" type="button" :id="`plus-${item.id}`">
                      <strong class="h5">+</strong>
                    </button>
                  </div>
                  </td>
          ...
          <div class="px-2 py-2 text-center" v-if="tmpCart.length===0">
            ...
            <router-link to="/product" class="btn btn-primary btn-hv-style">
              去逛逛<i class="fas fa-angle-double-right"></i>
            </router-link>
          </div>
        </form>
      </div>
